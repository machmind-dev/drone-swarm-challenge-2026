...
| 20:43 `..._2043_drone3` | 3 | `log_28` | ❌ Flew +X / LH, landed | **First vision fix after 79 s of flow-only flight reset pos ~5 m in x** (Δ(−2.8,−0.75)+(−2.43,−1.08)); drone chased phantom error +X and −Y. `cs_ev_pos=0` until 79 s. Keyboard log confirms commanded arena (1,3)→(6.5,3); `ned_offset≈(1,3)` correct during flow phase. |
...
| *(uncommitted at pause)* | P4 | **±30° viewing-incidence gate** (`MAX_VIEW_ANGLE_DEG`) — reject oblique markers | ❌ not built/flashed; value under review (OPEN-3) |
...
| OPEN-3 | **Vision coverage is the root issue** — no fusion for 79 s (20:43) / 19 s gap (19:44). | **User decision: fix coverage first.** Investigate marker layout / camera FOV / flight path / range+angle gates before tuning incidence. The ±30° gate is in tension with coverage (tighter → more dropouts → bigger resets); likely relax to 45° or hold until coverage improves. |
