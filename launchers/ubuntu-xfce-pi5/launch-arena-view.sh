...
if [ -z "$MACHMIND_ARENA_TERMINAL" ]; then
    export MACHMIND_ARENA_TERMINAL=1
    export NO_AT_BRIDGE=1
    if command -v xfce4-terminal >/dev/null 2>&1; then
        exec xfce4-terminal --title="Mach Mind - Arena View" --hold --command="env MACHMIND_ARENA_TERMINAL=1 bash '$0' $*"
    elif command -v gnome-terminal >/dev/null 2>&1; then
        exec gnome-terminal --title="Mach Mind - Arena View" -- bash "$0" "$@"
    fi
fi
